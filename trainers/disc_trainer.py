...
from models.utils import save_obj_mesh_with_color, calculate_psnr, draw_joint
...
class DiscTrainer:
    def __init__(self, conf, mano_layer):
        ...
        self.use_blur = conf.get_bool('use_blur', False)
        if self.use_blur:
    ...
    def initialize_model(self, pretrain_path_giif=None, pretrain_path_disc=None):
        ...
        if pretrain_path_disc:
            state_dict = torch.load(pretrain_path_disc).state_dict()
            print('Loading Pretrained Disc from ', pretrain_path_disc)
    ...
    def pretrain(self, giif_model=None):
        ...
        for epoch in range(self.giif_epoch):
            ...
            for i, data in pbar:
                ...
                #  = F.l1_loss(F.interpolate(out, self.lr_size), lr_img) * 0.1
                tot_loss = tot_loss + loss_l1 + disc_loss
                    
                # if epoch > 5:
                #     src_code = self.model.id_encoder(out.clone())
                ...
                if i % 200 == 0:
                    ...
                    lr_img = F.interpolate(lr_img, src_img.shape[2])
                    normal = render_img[:, :3]
                    board = torch.zeros_like(normal).cuda()
                    normal = board + (0.5 + normal / 2) * src_mask
                    render_img = F.normalize(normal, p=2, dim=1)
                    save_img = torch.cat([src_ori_img, normal], 3)
                   
                    if self.use_depths:
                        depth = render_img[:, 3:]
                        save_img = torch.cat([save_img, depth], 3)
                    
                    save_img = torch.cat([save_img, lr_img, out], 3)
    ...
    def eval(self, ):
        ...
        for i, data in pbar:
            ...
            #     'img': pair_img,
            #     'pose': pair_pose,
            #     'joints25d': pair_joints25d,
            #     'joint_img': pair_joints_img,
            #     'w2c': pair_w2c,
            #     'rays': pair_rays,
            #     'proj': pair_proj,
            # }
            ...
            # mano_psnr = calculate_psnr(gt_img[0].permute(1, 2, 0).detach().cpu().numpy(), np.clip(render_img[0].permute(1, 2, 0).detach().cpu().numpy(), 0, 1), src_mask[0].detach().cpu().numpy()) 
            
            psnrs.append(psnr)
            # mano_psnrs.append(mano_psnr)
            ...
            if i % 10 == 0: 
                board = torch.zeros_like(src_ori_img)
                lr_img = F.interpolate(lr_img, src_ori_img.shape[-1])
                src_cond_img = []
                if self.use_joint:
