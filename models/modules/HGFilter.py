...
class HGFilter(nn.Module):
    def __init__(self, in_channel = 3, num_stack=4, n_feat=256, norm='group', hg_down='ave_pool', num_hourglass=2, hourglass_dim=256):
        ...
        self.conv4 = ConvBlock(128, n_feat, self.norm)

        # Stacking part
        for hg_module in range(self.num_modules):
            self.add_module('m' + str(hg_module), HourGlass(1, num_hourglass, n_feat, self.norm))

            self.add_module('top_m_' + str(hg_module), ConvBlock(n_feat, n_feat, self.norm))
            self.add_module('conv_last' + str(hg_module),
                            nn.Conv2d(n_feat, n_feat, kernel_size=1, stride=1, padding=0))
            if self.norm == 'batch':
                self.add_module('bn_end' + str(hg_module), nn.BatchNorm2d(n_feat))
            elif self.norm == 'group':
                self.add_module('bn_end' + str(hg_module), nn.GroupNorm(32, n_feat))
                
            self.add_module('l' + str(hg_module), nn.Conv2d(n_feat,
                                                            hourglass_dim, kernel_size=1, stride=1, padding=0))

            if hg_module < self.num_modules - 1:
                self.add_module(
                    'bl' + str(hg_module), nn.Conv2d(n_feat, n_feat, kernel_size=1, stride=1, padding=0))
                self.add_module('al' + str(hg_module), nn.Conv2d(hourglass_dim,
                                                                 n_feat, kernel_size=1, stride=1, padding=0))
