...
def load_obj_mesh_with_color(mesh_path):
    verts = []
    colors = []
    faces = []
    ...
    with open(mesh_path, 'r') as file:
        for line in file:
            ...
            if parts[0] == 'v':  # Vertex data
                v = list(map(float, parts[1:4]))  # x, y, z
                c = list(map(float, parts[4:7]))  # r, g, b
                verts.append(v)
                colors.append(c)
            elif parts[0] == 'f':  # Face data
                f = list(map(lambda x: int(x) - 1, parts[1:4]))  # Convert to 0-based index
                faces.append(f)
...
COLORMAP = {
    "thumb": {"ids": [0, 1, 2, 3], "color": "g", 'color_val': [0, 255, 0]},
    "index": {"ids": [0, 4, 5, 6], "color": "c", 'color_val': [255, 255, 0]},
    "middle": {"ids": [0, 7, 8, 9], "color": "b", 'color_val': [255, 0, 0]},
    "ring": {"ids": [0, 10, 11, 12], "color": "m", 'color_val': [0, 255, 255]},
    "little": {"ids": [0, 13, 14, 15], "color": "r", 'color_val': [0, 0, 255]},
}
...
def draw_joint(joint, board, K=None):
    device = board.device
    # 3D Joint
    if K is not None:
        joint = projectPoints(joint, K)
    image_raw = (board.permute(1, 2, 0).detach().cpu().numpy() * 255).astype(np.uint8).copy()
    img_size_x = board.size(-1)
    img_size_y = board.size(-2)
    
    joint = joint[:, :2]
    px_landmark = []
    
    for each_land in joint: 
        landmark_px = normalized_to_pixel_coordinates(each_land[0], each_land[1], img_size_x, img_size_y)
        px_landmark.append(landmark_px)
        
        # px_landmark.append(each_land)
    
    px_landmark = np.array(px_landmark)
    
    for finger, params in COLORMAP.items():
        points = [px_landmark[params['ids']]]
        cv2.polylines(image_raw, points, False, params['color_val'], 2)

    return torch.from_numpy(image_raw).permute(2, 0, 1).float().to(device)
...
def calculate_psnr(img1, img2, mask=None):
    ...
    Args:
        img1 (np.ndarray): First image (H, W, C), range [0, 1].
        img2 (np.ndarray): Second image (H, W, C), range [0, 1].
        mask (np.ndarray): Binary mask (H, W), where 1 indicates valid pixels.
    ...
    # Compute masked MSE
    if mask is not None:
        # mse = np.sum(((img1[mask == 1] - img2[mask == 1]) ** 2)) / np.sum(mask)
    ...
    # Compute PSNR (Correct formula)
    return 20 * math.log10(1.0 / np.sqrt(mse))


def convert2video(filepath, images, fps=30, ext=['mp4', 'gif']):
    # with imageio.get_writer(filepath, fps=fps) as video:
    vieo_data = []
    for image in images:
        frame = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        vieo_data.append(frame)
    for ex in ext:
        imageio.mimwrite(filepath + '.' + ex, vieo_data, fps=fps)
        
    
            
            
def proj_verts2coord(proj_verts):
    coord = torch.zeros_like(proj_verts).cuda()
    coord[..., 0] = proj_verts[..., 0] / proj_verts[..., 3] + 1
    coord[..., 1] = proj_verts[..., 1] / proj_verts[..., 3] + 1
    
    return coord[..., :2] / 2


def tensor2img(t, idx=0):
    n = t.dim()
    if n == 4:
        t = t[idx]
    ...
    if t.shape[0] == 3:
        t = t.permute(1, 2, 0)
    return (t.clamp(0, 1).detach().cpu().numpy() * 255).astype(np.uint8)


from collections import deque
class tensorQueue2(object):
    def __init__(self, size):
        self.queue1 = deque(maxlen=size)
        self.queue2 = deque(maxlen=size)

    def next(self, val1, val2):
        """
        :type val: int
        :rtype: float
        """
        self.queue1.append(val1)
        self.queue2.append(val2)
        
        return torch.cat(list(self.queue1), dim=0), torch.cat(list(self.queue2), dim=0) 